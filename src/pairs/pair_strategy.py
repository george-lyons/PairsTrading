...
class PairStrategy(object):
    ...
    def __init__(self, benchmark_series, y_series, x_series, start_full, end_full, start_train, end_train, trading_fee=0.01, freq='daily', normalize_window=True, tau_denom=365, rf=0.02):
        ...
        self.waiting_period = self.WAITING_PERIOD
    ...
    def plot_pair2(self):
        ...
        ax_spread.plot(self.spread[-self.tau_denom:])
        ax_spread.axhline(self.upper_bound, linestyle='--', color="g", label="Trading bound")
        ax_spread.axhline(self.lower_bound, linestyle='--', color="g")
        ax_spread.axhline(2 * self.upper_bound, linestyle='--', color="r", label="Stop loss")
        ax_spread.axhline(2 * self.lower_bound, linestyle='--', color="r")

        ax_spread.plot_date([self.spread.index[i] for i in self.open_positions],
                            [self.spread[i] for i in self.open_positions], label='Open position',
                            marker='^', markeredgecolor='b', markerfacecolor='b', markersize=16)
        ax_spread.plot_date([self.spread.index[i] for i in self.closed_positions],
                            [self.spread[i] for i in self.closed_positions], label='Closed position',
                            marker='P', markeredgecolor='g', markerfacecolor='g', markersize=16)
        ax_spread.plot_date([self.spread.index[i] for i in self.stop_losses],
                            [self.spread[i] for i in self.stop_losses], label='Stop loss',
    ...
    def __calculate_returns(self, pos_x, pos_y, i):
        if pos_x[2] == 'l':
            ...
            profit = (self.x_full[i] - self.pos_x[0]) * pos_x[1] + (self.pos_y[0] - self.y_full[i]) * \
                     pos_y[1]
        else:
            # long on Y
            cost_long = pos_y[0] * pos_y[1]
            profit = (self.pos_x[0] - self.x_full[i]) * pos_x[1] + (self.y_full[i] - self.pos_y[0]) * \
                     pos_y[1]
    ...
    def generate_trading_signals(self, z=1):
        self.sd = z
        data = self.data
        ...
        self.upper_bound = mu + sigma * z
        self.lower_bound = mu - sigma * z
        self.upper_trading = self.__level_crosses(spread, level=self.upper_bound)
        self.lower_trading = self.__level_crosses(spread, level=self.lower_bound)
        self.upper_stop = self.__level_crosses(spread, level=2*self.upper_bound)
        self.lower_stop = self.__level_crosses(spread, level=2*self.lower_bound)
        self.mean = self.__level_crosses(spread, level=mu)
        ...
        while i < len(spread):
            if open_position:
                ...
                if self.upper_trading[i] == -1 or self.lower_trading[i] == 1:
                    ...
                    open_price_x = self.x_full[i]
                    open_price_y = self.y_full[i]
                    b = self.b
        ...
        self.returns_series = pd.Series(index=self.x_full.index, data=returns)
